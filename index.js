...
async function recoverDatabaseFromChannel(guild) {
    const profileChannelId = process.env.PROFILE_CHANNEL_ID;
    if (!profileChannelId) return;

    const channel = await guild.channels.fetch(profileChannelId).catch(() => null);
    if (!channel || !channel.isTextBased()) return;

    console.log("🔍 Đang tiến hành quét và phân tích tin nhắn trên sảnh danh vọng để khôi phục cơ sở dữ liệu...");

    // Tải 100 tin nhắn gần nhất từ sảnh danh vọng
    const messages = await channel.messages.fetch({ limit: 100 }).catch(() => null);
    if (!messages) return;

    const recoveredData = {};
    // Nạp đầy đủ danh sách thành viên để đối chiếu ID chính xác
    await guild.members.fetch().catch(() => null);

    for (const [msgId, message] of messages) {
        if (message.author.id !== client.user.id || message.embeds.length === 0) continue;

        const embed = message.embeds[0];
        if (!embed.title || !embed.author || !embed.author.name) continue;

        // 1. Tìm userId dựa trên author tag/username
        const authorTag = embed.author.name;
        const member = guild.members.cache.find(m => m.user.tag === authorTag || m.user.username === authorTag);
        if (!member) continue;
        const userId = member.id;

        // 2. Phân tích thông tin từ Embed
        const name = embed.title.replace(/☁️/g, "").trim();
        const desc = embed.description || "";

        let slogan = "Chưa có câu nói tâm đắc";
        const sloganMatch = desc.match(/> 💬 \*(.*?)\*/);
        if (sloganMatch) {
            slogan = sloganMatch[1].replace(/"/g, "").trim();
        }

        let location = "Chưa rõ";
        const locationMatch = desc.match(/📌 \*\*Nơi ở:\*\* (.*)/);
        if (locationMatch) {
            location = locationMatch[1].trim();
        }

        let hobbies = "Chưa rõ";
        const hobbiesMatch = desc.match(/🩷 \*\*Sở thích:\*\* (.*)/);
        if (hobbiesMatch) {
            hobbies = hobbiesMatch[1].trim();
        }

        let day = null, month = null, year = null;
        const bdayMatch = desc.match(/🎂 \*\*Ngày sinh:\*\* (\d+)\/(\d+)(?:\/(\d+))?/);
        if (bdayMatch) {
            day = parseInt(bdayMatch[1], 10);
            month = parseInt(bdayMatch[2], 10);
            if (bdayMatch[3]) year = parseInt(bdayMatch[3], 10);
        }
    ...
    if (Object.keys(recoveredData).length > 0) {
        ...
        saveData(data);
        console.log(`🎉 Khôi phục hoàn tất thành công ${Object.keys(recoveredData).length} hồ sơ thành viên vào file JSON!`);
    } else {
        console.log("⚠️ Không tìm thấy tin nhắn profile cũ nào hợp lệ trên sảnh danh vọng.");
    }
}
...
client.once("ready", async () => {
    ...
    // 🔥 KÍCH HOẠT TỰ KHÔI PHỤC KHI PHÁT HIỆN FILE TRỐNG, SAU ĐÓ HỒI SINH ẢNH 🔥
    try {
        const data = loadData();
        const guilds = await client.guilds.fetch();
        ...
        for (const [guildId] of guilds) {
            ...
            if (guild) {
                // Nếu file dữ liệu bị trống rỗng {}, khởi động cơ chế tự sửa sảnh
                if (Object.keys(data).length === 0) {
                    await recoverDatabaseFromChannel(guild);
                }
                
                console.log("⏳ Bắt đầu làm mới và sửa toàn bộ ảnh hết hạn tại Sảnh Danh Vọng...");
                await syncAllProfiles(guild);
            }
        }
        console.log("✅ Hoàn tất quá trình đồng bộ và khôi phục ảnh cũ khi khởi động!");
    } catch (startSyncErr) {
