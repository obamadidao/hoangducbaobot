...
client.once("ready", async () => {
    ...
    // 🔥 TỰ ĐỘNG QUÉT VÀ HỒI SINH ẢNH TOÀN BỘ SẢNH DANH VỌNG KHI KHỞI ĐỘNG BOT 🔥
    try {
        console.log("⏳ Khởi động hệ thống: Đang tự động làm mới và sửa toàn bộ ảnh hết hạn tại Sảnh Danh Vọng...");
        const guilds = await client.guilds.fetch();
        for (const [guildId] of guilds) {
        ...
        console.log("✅ Hoàn tất quá trình hồi sinh toàn bộ ảnh cũ khi khởi động!");
    } catch (startSyncErr) {
