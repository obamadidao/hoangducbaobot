...
const tempEventRoles = new Map(); // Lưu tạm role khi tạo sự kiện
...
client.once("ready", async () => {
    ...
    const commands = [
        ...
        new SlashCommandBuilder()
            ...
            .setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages),
        // CÁC LỆNH VỀ SỰ KIỆN MỚI TÍCH HỢP
        new SlashCommandBuilder()
            .setName('taosukien')
            .setDescription('Tạo sự kiện nhắc nhở mới và gửi lời chúc khi đến ngày')
            .addRoleOption(option => option.setName('role1').setDescription('Role 1 cần nhắc đến khi diễn ra sự kiện').setRequired(false))
            .addRoleOption(option => option.setName('role2').setDescription('Role 2 cần nhắc đến khi diễn ra sự kiện').setRequired(false)),
        new SlashCommandBuilder()
            .setName('danhsachsukien')
            .setDescription('Xem danh sách tất cả các sự kiện đã lưu (Chỉ dùng ở kênh Setup)'),
        new SlashCommandBuilder()
            .setName('sukien')
            .setDescription('Tra cứu thông tin chi tiết của một sự kiện cụ thể (Có thể dùng ở mọi kênh)'),
        new SlashCommandBuilder()
            .setName('suasukien')
            .setDescription('Chỉnh sửa thông tin và lời chúc của một sự kiện đã tạo'),
        new SlashCommandBuilder()
            .setName('xoasukien')
            .setDescription('Xóa vĩnh viễn sự kiện khỏi danh sách nhắc nhở')
                    ...
                    try {
                        ...
                        if (newsChannel) {
                            ...
                            const eventEmbed = new EmbedBuilder()
                                ...
                                .setFooter({ text: "Sự kiện được lập lịch nhắc nhở tự động 📅" })
                                .setTimestamp();

                            await newsChannel.send({ content: mentionString || null, embeds: [eventEmbed] });
                        }
                    } catch (err) { console.error("Lỗi khi chạy cron job thông báo sự kiện:", err); }
                }
            }
        }
    });
...
async function createRegisteredEventsSelectMenu(interaction, customId, placeholder, restrictToUser = false) {
    const eventsData = loadEventsData();
    ...
    let filteredIds = eventIds;
    if (restrictToUser && !isAdmin) {
        filteredIds = eventIds.filter(id => eventsData[id].creatorId === interaction.user.id);
    }
    ...
    for (const id of filteredIds) {
        ...
        options.push({
            ...
            value: id,
            description: `Ngày diễn ra: ${event.day}/${event.month} | Tác giả: ${event.creatorName || "Ẩn danh"}`.slice(0, 100)
        });
    }
...
client.on("interactionCreate", async interaction => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        // --- LỆNH PUBLIC BẤT KỲ ĐÂU (Bypass check) ---
        ...
        if (commandName === "sukien") {
            ...
            if (eventIds.length === 0) {
                return interaction.reply({ content: "❌ Hiện tại chưa có sự kiện nhắc nhở nào được lưu!", flags: ['Ephemeral'] });
            }

            const selectMenu = new StringSelectMenuBuilder()
                .setCustomId("select_event_lookup")
                .setPlaceholder("Chọn sự kiện để tra cứu thông tin...");
            ...
            for (const id of eventIds) {
                ...
                options.push({
                    label: event.name.slice(0, 100),
                    value: id,
                    description: `Discord ID sự kiện: ${id}`
                });
            }
            ...
            const row = new ActionRowBuilder().addComponents(selectMenu);
            return await interaction.reply({ content: "📅 **Chọn sự kiện bạn muốn tra cứu thông tin:**", components: [row] });
        }

        // --- RÀNG BUỘC KÊNH ĐIỀN THÔNG TIN ---
        const restrictedCommands = ["birthdays", "taoprofile", "taohoprofile", "suaprofile", "suaanh", "xoaprofile", "taosukien", "danhsachsukien", "suasukien", "xoasukien"];
        ...
        if (commandName === "taosukien") {
            if (!isSelfAllowed(member)) return interaction.reply({ content: "❌ Bạn không có quyền khởi tạo sự kiện!", flags: ['Ephemeral'] });

            const role1 = interaction.options.getRole('role1')?.id;
            const role2 = interaction.options.getRole('role2')?.id;
            const targetRoles = [role1, role2].filter(Boolean);

            tempEventRoles.set(user.id, targetRoles);

            const modal = new ModalBuilder().setCustomId(`event_modal_${user.id}`).setTitle("Tạo Sự Kiện Nhắc Nhở Mới");
            const nameInput = new TextInputBuilder().setCustomId("modal_event_name").setLabel("Tên sự kiện").setStyle(TextInputStyle.Short).setRequired(true);
            const dateInput = new TextInputBuilder().setCustomId("modal_event_date").setLabel("Ngày diễn ra (Vd: 14/02 hoặc 25/12)").setStyle(TextInputStyle.Short).setRequired(true);
            const msgInput = new TextInputBuilder().setCustomId("modal_event_message").setLabel("Nội dung lời chúc / thông điệp nhắc nhở").setStyle(TextInputStyle.Paragraph).setRequired(true);
        ...
        if (commandName === "danhsachsukien") {
            ...
            for (const id in eventsData) {
                ...
                const countdownText = daysLeft === 0 ? "🔔 Hôm nay diễn ra!" : `Còn **${daysLeft} ngày**`;
                listText += `#${index} 📅 **${event.name}**\n📆 Ngày: ${event.day}/${event.month}\n⏳ ${countdownText}\n✍️ Người tạo: <@${event.creatorId}>\n\n`;
                index++;
            }
        ...
        if (commandName === "suasukien") {
            if (!isSelfAllowed(member)) return interaction.reply({ content: "❌ Bạn không có quyền chỉnh sửa sự kiện!", flags: ['Ephemeral'] });

            const selectMenu = await createRegisteredEventsSelectMenu(interaction, "select_edit_event", "Chọn sự kiện cần sửa...", true);
            if (!selectMenu) return interaction.reply({ content: "❌ Bạn không có sự kiện nào hoặc danh sách sự kiện trống!", flags: ['Ephemeral'] });

            const row = new ActionRowBuilder().addComponents(selectMenu);
            return await interaction.reply({ content: "⚙️ **Chọn sự kiện bạn muốn chỉnh sửa nội dung:**", components: [row], flags: ['Ephemeral'] });
        }

        // --- LỆNH /XOASUKIEN ---
        if (commandName === "xoasukien") {
            if (!isSelfAllowed(member)) return interaction.reply({ content: "❌ Bạn không có quyền xóa sự kiện!", flags: ['Ephemeral'] });

            const selectMenu = await createRegisteredEventsSelectMenu(interaction, "select_delete_event", "Chọn sự kiện cần xóa vĩnh viễn...", true);
            if (!selectMenu) return interaction.reply({ content: "❌ Bạn không có sự kiện nào để xóa!", flags: ['Ephemeral'] });

            const row = new ActionRowBuilder().addComponents(selectMenu);
            return await interaction.reply({ content: "🚨 **Chọn sự kiện bạn muốn xóa bỏ khỏi hệ thống nhắc nhở:**", components: [row], flags: ['Ephemeral'] });
        }
    ...
    if (interaction.isStringSelectMenu()) {
        ...
        // --- XỬ LÝ MENU TRA CỨU SỰ KIỆN (/SUKIEN) ---
        if (customId === "select_event_lookup") {
            const eventsData = loadEventsData();
            const event = eventsData[targetId];

            if (!event) {
                return interaction.reply({ content: "❌ Sự kiện không tồn tại trên hệ thống!", flags: ['Ephemeral'] });
            }

            const daysLeft = getDaysUntilBirthday(event.day, event.month);
            const countdownText = daysLeft === 0 ? "🔔 Hôm nay diễn ra!" : `Còn **${daysLeft} ngày**`;

            let roleMentions = "Không nhắc đến ai";
            if (event.targetRoles && event.targetRoles.length > 0) {
                roleMentions = event.targetRoles.map(r => `<@&${r}>`).join(", ");
            }

            const lookupEmbed = new EmbedBuilder()
                .setColor("#F1C40F")
                .setTitle(`🔔 THÔNG TIN SỰ KIỆN: ${event.name} 🔔`)
                .setDescription(`📅 **Ngày diễn ra:** ${event.day}/${event.month}\n⏳ **Thời gian:** ${countdownText}\n👥 **Role nhận thông báo:** ${roleMentions}\n\n💬 **Thông điệp nhắc nhở:**\n${event.message}`)
                .setFooter({ text: `Tạo bởi: ${event.creatorName}` })
                .setTimestamp();

            return await interaction.update({ content: null, embeds: [lookupEmbed], components: [] });
        }

        // --- SỬA SỰ KIỆN CHỌN TỪ MENU ---
        if (customId === "select_edit_event") {
            const eventsData = loadEventsData();
            const event = eventsData[targetId];

            if (!event) return interaction.reply({ content: "❌ Lỗi: Sự kiện không tồn tại.", flags: ['Ephemeral'] });

            const modal = new ModalBuilder()
                .setCustomId(`edit_event_modal_${targetId}`)
                .setTitle(`Sửa Sự Kiện: ${event.name}`);

            const nameInput = new TextInputBuilder()
                .setCustomId("modal_event_name").setLabel("Tên sự kiện").setStyle(TextInputStyle.Short).setValue(event.name).setRequired(true);

            const dateInput = new TextInputBuilder()
                .setCustomId("modal_event_date").setLabel("Ngày diễn ra (Vd: 14/02)").setStyle(TextInputStyle.Short).setValue(`${event.day}/${event.month}`).setRequired(true);

            const msgInput = new TextInputBuilder()
                .setCustomId("modal_event_message").setLabel("Thông điệp nhắc nhở").setStyle(TextInputStyle.Paragraph).setValue(event.message).setRequired(true);
        ...
        // --- XÓA SỰ KIỆN CHỌN TỪ MENU ---
        if (customId === "select_delete_event") {
            const eventsData = loadEventsData();
            if (!eventsData[targetId]) return interaction.reply({ content: "❌ Lỗi: Sự kiện không tồn tại.", flags: ['Ephemeral'] });

            const eventName = eventsData[targetId].name;
            delete eventsData[targetId];
            saveEventsData(eventsData);

            return await interaction.update({ content: `✅ Đã xóa thành công sự kiện nhắc nhở **${eventName}** khỏi hệ thống!`, components: [] });
        }
    ...
    if (interaction.isModalSubmit()) {
        ...
        // --- SUBMIT MODAL TẠO SỰ KIỆN ---
        if (customId.startsWith("event_modal_")) {
            await interaction.deferReply({ flags: ['Ephemeral'] });
            
            const eventName = fields.getTextInputValue("modal_event_name");
            const eventDate = fields.getTextInputValue("modal_event_date").trim();
            const eventMsg = fields.getTextInputValue("modal_event_message");
            ...
            if (isNaN(day) || isNaN(month) || day < 1 || day > 31 || month < 1 || month > 12) {
                return interaction.editReply({ content: "❌ Định dạng ngày tháng sự kiện không hợp lệ! Vui lòng nhập dạng DD/MM (Vd: 14/02)." });
            }

            const targetRoles = tempEventRoles.get(interaction.user.id) || [];
            tempEventRoles.delete(interaction.user.id);
            ...
            saveEventsData(eventsData);
            return await interaction.editReply({ content: `✅ Đã lưu và khởi tạo sự kiện nhắc nhở **${eventName}** thành công cho ngày ${day}/${month}!` });
        }

        // --- SUBMIT MODAL SỬA SỰ KIỆN ---
        if (customId.startsWith("edit_event_modal_")) {
            ...
            if (isNaN(day) || isNaN(month) || day < 1 || day > 31 || month < 1 || month > 12) {
                return interaction.editReply({ content: "❌ Định dạng ngày tháng sửa đổi không hợp lệ!" });
            }

            const eventsData = loadEventsData();
            const existing = eventsData[eventId];

            if (!existing) return interaction.editReply({ content: "❌ Không tìm thấy sự kiện cần chỉnh sửa." });
            ...
            saveEventsData(eventsData);
            return await interaction.editReply({ content: `✅ Đã sửa đổi và lưu thành công thông tin sự kiện **${eventName}**!` });
        }
