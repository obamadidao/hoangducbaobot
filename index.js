...
client.once("ready", async () => {
    ...
    // ==========================================
    // CẤU HÌNH LỜI CHÚC & CRON JOB SINH NHẬT / SỰ KIỆN
    // ==========================================
    ...
    cron.schedule("0 0 * * *", async () => {
        ...
        for (const [guildId] of guilds) {
            ...
            // 1. Quét thông báo chúc mừng sinh nhật
            ...
            // 2. Quét thông báo sự kiện nhắc nhở trong Server
            for (const eventId in eventsData) {
                const event = eventsData[eventId];
                if (event.day === day && event.month === month) {
                    let rolesTagText = "";
                    if (event.roles && event.roles.length > 0) {
                        rolesTagText = event.roles.map(rId => `<@&${rId}>`).join(" ");
                    }

                    const newsChannelId = "1313150267881033739"; // Kênh tin tức chỉ định
...
client.on("interactionCreate", async interaction => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        // Ràng buộc các lệnh cấu hình chỉ được phép thực hiện tại Setup Channel
        ...
        // --- LỆNH /BIRTHDAY (CÁ NHÂN, CÔNG KHAI 100%, DÙNG ĐƯỢC Ở MỌI KÊNH) ---
        if (commandName === "birthday") {
            const data = loadData();
            const userIds = Object.keys(data);

            if (userIds.length === 0) {
                return interaction.reply({ content: "❌ Hiện chưa có thành viên nào khởi tạo hồ sơ sinh nhật cả!", flags: ['Ephemeral'] });
            }
            ...
            for (const id of userIds) {
                ...
                options.push({
                    label: `${userData.name}${discordTag}`.slice(0, 100),
                    value: id,
                    description: `Discord ID: ${id}` // Hiển thị ID để người dùng bất ngờ khi click xem
                });
            }
            ...
            const row = new ActionRowBuilder().addComponents(selectMenu);
            return await interaction.reply({ content: "🎂 **Chọn thành viên bạn muốn xem thông tin sinh nhật:**", components: [row] });
        }
        ...
        if (commandName === "taosukien") {
            ...
            const rolesSelected = [];
            if (role1) rolesSelected.push(role1.id);
            if (role2) rolesSelected.push(role2.id);
            ...
            const eventName = new TextInputBuilder().setCustomId("modal_sukien_name").setLabel("Tên sự kiện").setStyle(TextInputStyle.Short).setRequired(true);
            const eventDate = new TextInputBuilder().setCustomId("modal_sukien_date").setLabel("Ngày diễn ra (Vd: 14/02 hoặc 20/10)").setStyle(TextInputStyle.Short).setRequired(true);
            const eventMsg = new TextInputBuilder().setCustomId("modal_sukien_msg").setLabel("Nội dung lời nhắn thông báo").setStyle(TextInputStyle.Paragraph).setRequired(true);

            modal.addComponents(
                new ActionRowBuilder().addComponents(eventName),
                new ActionRowBuilder().addComponents(eventDate),
                new ActionRowBuilder().addComponents(eventMsg)
            );
        ...
        if (commandName === "suasukien") {
            ...
            // Tìm những sự kiện được phép chỉnh sửa
            const editableEvents = eventIds.filter(id => {
                const event = eventsData[id];
                return isAdmin || event.creatorId === user.id;
            });

            if (editableEvents.length === 0) {
                return interaction.reply({ content: "❌ Bạn không có sự kiện nào được tạo lập để có thể chỉnh sửa!", flags: ['Ephemeral'] });
            }

            const selectMenu = new StringSelectMenuBuilder()
                .setCustomId("select_event_edit")
                .setPlaceholder("Chọn sự kiện bạn muốn chỉnh sửa...");

            const options = [];
            for (const id of editableEvents) {
                const event = eventsData[id];
                options.push({
                    label: event.name.slice(0, 100),
                    value: id,
                    description: `Ngày: ${event.day}/${event.month}`
                });
            }

            selectMenu.addOptions(options.slice(0, 25));
            const row = new ActionRowBuilder().addComponents(selectMenu);
            return await interaction.reply({ content: "⚙️ **Chọn sự kiện bạn muốn sửa thông tin:**", components: [row], flags: ['Ephemeral'] });
        }
        ...
        if (commandName === "xoasukien") {
            ...
            const deletableEvents = eventIds.filter(id => {
                const event = eventsData[id];
                return isAdmin || event.creatorId === user.id;
            });

            if (deletableEvents.length === 0) {
                return interaction.reply({ content: "❌ Bạn không sở hữu sự kiện nào để tiến hành xóa bỏ!", flags: ['Ephemeral'] });
            }

            const selectMenu = new StringSelectMenuBuilder()
                .setCustomId("select_event_delete")
                .setPlaceholder("Chọn sự kiện bạn muốn xóa bỏ...");

            const options = [];
            for (const id of deletableEvents) {
                const event = eventsData[id];
                options.push({
                    label: event.name.slice(0, 100),
                    value: id,
            ...
            selectMenu.addOptions(options.slice(0, 25));
            const row = new ActionRowBuilder().addComponents(selectMenu);
            return await interaction.reply({ content: "🚨 **Chọn sự kiện bạn muốn xóa vĩnh viễn khỏi server:**", components: [row], flags: ['Ephemeral'] });
        }
        ...
        // --- LỆNH /SUKIEN (CÔNG KHAI 100%, DÙNG ĐƯỢC Ở MỌI KÊNH) ---
        if (commandName === "sukien") {
            ...
            for (const id of eventIds) {
                ...
                options.push({
                    label: event.name.slice(0, 100),
                    value: id,
                    description: `Người tạo: @${event.creatorName || "Ẩn danh"}`
                });
            }
    ...
    if (interaction.isStringSelectMenu()) {
        ...
        // --- XỬ LÝ TRA CỨU SINH NHẬT CÁ NHÂN (CÔNG KHAI) ---
        if (customId === "select_birthday_lookup") {
            ...
            if (!userData) {
                return await interaction.editReply({ content: "❌ Không tìm thấy thông tin hồ sơ của thành viên này." });
            }

            const userObj = client.users.cache.get(targetId) || await client.users.fetch(targetId).catch(() => null);
            const userTag = userObj ? `@${userObj.username}` : "Thành viên";
            ...
            const bdayEmbed = new EmbedBuilder()
                ...
                .setDescription(`👤 **${userData.name}** (${userTag})\n🎂 Ngày sinh: ${userData.day}/${userData.month}${userData.year ? `/${userData.year}` : ""}\n⏳ ${countdownText}`)
                .setThumbnail(userObj ? userObj.displayAvatarURL({ dynamic: true }) : null)
                .setTimestamp();

            // Sửa lại tin nhắn chính, làm sạch dropdown menu
            await interaction.editReply({ content: null, embeds: [bdayEmbed], components: [] });
        }

        // --- SỬA SỰ KIỆN MỞ MODAL ---
        if (customId === "select_event_edit") {
            const eventsData = loadEventsData();
            const event = eventsData[targetId];

            if (!event) return interaction.reply({ content: "❌ Sự kiện không tồn tại trên hệ thống!", flags: ['Ephemeral'] });

            const modal = new ModalBuilder().setCustomId(`edit_event_modal_${targetId}`).setTitle("Chỉnh Sửa Sự Kiện");
            const nameInput = new TextInputBuilder().setCustomId("modal_sukien_name").setLabel("Tên sự kiện").setStyle(TextInputStyle.Short).setValue(event.name || "").setRequired(true);
            const dateInput = new TextInputBuilder().setCustomId("modal_sukien_date").setLabel("Ngày diễn ra (Vd: 14/02)").setStyle(TextInputStyle.Short).setValue(`${event.day}/${event.month}`).setRequired(true);
            const msgInput = new TextInputBuilder().setCustomId("modal_sukien_msg").setLabel("Nội dung lời nhắn thông báo").setStyle(TextInputStyle.Paragraph).setValue(event.message || "").setRequired(true);
        ...
        if (customId === "select_event_delete") {
            ...
            if (!eventsData[targetId]) {
                return await interaction.editReply({ content: "❌ Sự kiện này đã bị xóa hoặc không còn tồn tại!" });
            }
        ...
        // --- XỬ LÝ TRA CỨU SỰ KIỆN CÁ NHÂN (CÔNG KHAI) ---
        if (customId === "select_event_lookup") {
            ...
            if (!event) {
                return await interaction.editReply({ content: "❌ Không tìm thấy thông tin sự kiện này trên hệ thống." });
            }

            const daysLeft = getDaysUntilBirthday(event.day, event.month);
            const countdownText = daysLeft === 0 ? "🔔 Hôm nay luôn! Đang diễn ra!" : `Còn **${daysLeft} ngày**`;

            let rolesTagText = "Không có";
            if (event.roles && event.roles.length > 0) {
                rolesTagText = event.roles.map(rId => `<@&${rId}>`).join(", ");
            }
    ...
    if (interaction.isModalSubmit()) {
        ...
        // --- SUBMIT MODAL TẠO SỰ KIỆN NHẮC NHỞ ---
        if (customId === "create_event_modal") {
            ...
            if (sDateRaw) {
                ...
                if (isNaN(day) || isNaN(month) || day < 1 || day > 31 || month < 1 || month > 12) {
                    return interaction.editReply({ content: "❌ Định dạng ngày tháng sự kiện không hợp lệ! Vui lòng nhập dạng: ngày/tháng (Vd: 14/02)" });
                }
            } else {
                return interaction.editReply({ content: "❌ Ngày diễn ra sự kiện không thể để trống." });
            }
            ...
            return await interaction.editReply({ content: `✅ Đã thiết lập thành công sự kiện: **${sName}** diễn ra vào ngày **${day}/${month}**!` });
        }

        // --- SUBMIT MODAL SỬA SỰ KIỆN NHẮC NHỞ ---
        if (customId.startsWith("edit_event_modal_")) {
            ...
            if (sDateRaw) {
                ...
                if (isNaN(day) || isNaN(month) || day < 1 || day > 31 || month < 1 || month > 12) {
                    return interaction.editReply({ content: "❌ Định dạng ngày tháng sự kiện sửa không hợp lệ!" });
                }
            }
            ...
            if (!existingEvent) {
                return interaction.editReply({ content: "❌ Lỗi: Sự kiện không còn tồn tại trên máy chủ." });
            }

            eventsData[targetEventId] = {
                ...existingEvent,
                name: sName,
                day: day,
                month: month,
            ...
            return await interaction.editReply({ content: `✅ Đã cập nhật thành công sự kiện: **${sName}**!` });
        }
